...
function somarProdutos(value) {
    for (i in value) {
        if (value[i].category == "Acessórios") {
            var somaAcessorio = value[i].price * value[i].quantity;
        }
        if (value[i].category == "Eletrônicos") {
            var somaEletronico = value[i].price * value[i].quantity;
        }
        if (value[i].category == "Eletrodomésticos") {
            var somaEletrodomesticos = value[i].price * value[i].quantity;
        }
        if (value[i].category == "Panelas") {
            var somaPanelas = value[i].price * value[i].quantity;
        }
    }
    console.log(
        "| Categorias ---------- R$Total\n\n" +
        "| Acessórios: --------- R$" + somaAcessorio + "\n" +
        "| Eletrodomésticos: --- R$" + somaEletrodomesticos + "\n" +
        "| Eletrônicos: -------- R$" + somaEletronico + "\n" +
        "| Panelas: ------------ R$" + somaPanelas + "\n"
    );

}

//Chamar funções
const newData = ordenarValores(data);

const finalData = organizaBanco(newData);
